...
\newcommand{\ect}{\sigma}
\newcommand{\med}{M\!e}
\docpart{Facile — Corrigé — Thème 2}
\begin{document}
\bandeau{Thème 2 — Mode et moyenne}{Niveau Facile — corrigé}

\begin{corbox}{Corrigé 1 — Mode (série discrète)}
Le plus grand effectif est $12$, obtenu par la pointure $39$. Donc $M=\mathbf{39}$
(et non $M=12$, qui est l'effectif).
\end{corbox}

\begin{corbox}{Corrigé 2 — Mode (série en classes)}
Le plus grand effectif est $12$, sur la classe $[12,16[$. Classe modale : $M=\mathbf{[12,16[}$
(centre $14$).
\end{corbox}

\begin{corbox}{Corrigé 3 — Moyenne pondérée}
$N=2+3+5=10$.
\[
m=\frac{2\times10+3\times12+5\times15}{10}=\frac{20+36+75}{10}=\frac{131}{10}=\mathbf{13{,}1}.
\]
\end{corbox}

\begin{corbox}{Corrigé 4 — Ajouter une constante}
Transformation $x\mapsto x+3$ ($\alpha=1,\ \beta=3$) : $m'=m+3=12+3=\mathbf{15}$\,g.
\end{corbox}

\begin{corbox}{Corrigé 5 — Multiplier par un facteur}
Transformation $x\mapsto 0{,}5\,x$ ($\alpha=0{,}5,\ \beta=0$) : $m'=0{,}5\times m=0{,}5\times12=\mathbf{6}$.
\end{corbox}
\end{document}
