...
\section*{Exercices — Moyen : fonction négative et changement de signe}

\begin{center}\itshape\small
Ici la fonction est négative, ou change de signe : penser à la \textbf{valeur absolue} et à
\textbf{couper aux racines} (Chasles). L'aire finale doit être positive.
\end{center}

\begin{exo}[M1 — fonction négative]
Soit $f(x)=x^2-4$. Calculer l'aire du domaine délimité par $\mathcal{C}_f$, l'axe des abscisses et
les droites $x=-2$ et $x=2$.

\smallskip
\begin{center}
\begin{tikzpicture}
  \begin{axis}[width=8.2cm,height=4.2cm,axis lines=middle,
      xmin=-2.9,xmax=2.9,ymin=-4.6,ymax=1.4,xtick={-2,2},ytick={-4},
      xlabel={$x$},ylabel={$y$},
      every axis x label/.style={at={(current axis.right of origin)},anchor=west},
      every axis y label/.style={at={(current axis.above origin)},anchor=south}]
    \addplot[domain=-2.7:2.7,samples=80,courbe]{x^2-4};
    \addplot[name path=F,domain=-2:2,samples=50,draw=none,forget plot]{x^2-4};
    \path[name path=b] (axis cs:-2,0)--(axis cs:2,0);
    \addplot[areacol2!45] fill between[of=F and b];
    \node at (axis cs:0,-1.8){$\mathcal{A}$};
  \end{axis}
\end{tikzpicture}
\end{center}
\end{exo}

\begin{exo}[M2 — droite négative]
Soit $f(x)=-2x$. Calculer l'aire du domaine compris entre $\mathcal{C}_f$, l'axe des abscisses et
les droites $x=0$ et $x=3$.
\end{exo}

\begin{exo}[M3 — changement de signe]
Soit $f(x)=x^2-2x$.
\begin{enumerate}[label=\alph*),leftmargin=2em,topsep=2pt]
  \item Déterminer les racines de $f$ et le signe de $f$ sur $[0,3]$.
  \item En déduire l'aire du domaine délimité par $\mathcal{C}_f$ et l'axe des abscisses sur $[0,3]$.
\end{enumerate}
\end{exo}

\begin{exo}[M4 — changement de signe]
Soit $f(x)=x^2-4$. Calculer l'aire comprise entre $\mathcal{C}_f$ et l'axe des abscisses sur
l'intervalle $[0,3]$ (attention : $f$ change de signe sur cet intervalle).
\end{exo}

\begin{exo}[M5 — trois morceaux par symétrie]
Soit $f(x)=x^2-1$. Calculer l'aire totale comprise entre $\mathcal{C}_f$ et l'axe des abscisses sur
$[-2,2]$.
\end{exo}
