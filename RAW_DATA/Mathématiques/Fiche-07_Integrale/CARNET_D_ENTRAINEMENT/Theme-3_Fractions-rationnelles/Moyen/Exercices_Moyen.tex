...
\begin{document}
\section*{Thème 3 — Fractions rationnelles \quad\textbullet\quad Niveau Moyen}

\begin{center}\itshape
Numérateur non constant, ou dénominateur à factoriser d'abord.\\
Pour chaque exercice : décomposer, puis donner une primitive complète (avec $\ln$).
\end{center}

\begin{exo}[1]
Calculer $\displaystyle\int \frac{2x}{(x+1)(x+3)}\,\dd x$.
\end{exo}

\begin{exo}[2]
Calculer $\displaystyle\int \frac{2x-1}{(x+2)(x-3)}\,\dd x$.
\end{exo}

\begin{exo}[3]
Le dénominateur n'est pas factorisé. Factoriser $x^2-x-6$, puis calculer
$\displaystyle\int \frac{x+7}{x^2-x-6}\,\dd x$.
\end{exo}

\begin{exo}[4]
Factoriser $x^2-4$, puis calculer
$\displaystyle\int \frac{x}{x^2-4}\,\dd x$.
Simplifier le résultat en un seul logarithme.
\end{exo}

\begin{exo}[5]
Factoriser $x^2+x-2$, puis calculer
$\displaystyle\int \frac{3x+1}{x^2+x-2}\,\dd x$.
\end{exo}
