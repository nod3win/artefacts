...
\begin{document}
\titrecarnet{Thème 3 — Géométrie analytique des droites}{Corrigé — niveau Facile}{Détail des calculs.}

\exercice{1}
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item $a=\dfrac{y_B-y_A}{x_B-x_A}=\dfrac{8-2}{4-1}=\dfrac{6}{3}=\boxed{2}$.
  \item $b=y_A-a\,x_A=2-2\times 1=0$. Donc $(AB):\ \boxed{y=2x}$.
\end{enumerate}

\exercice{2}
$M\Big(\dfrac{-2+4}{2};\dfrac{3+(-1)}{2}\Big)=\Big(\dfrac{2}{2};\dfrac{2}{2}\Big)=\boxed{(1;1)}$.

\exercice{3}
$\overline{AB}=\sqrt{(3-0)^2+(4-0)^2}=\sqrt{9+16}=\sqrt{25}=\boxed{5}$.

\exercice{4}
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Pente $3$, ordonnée à l'origine $-2$ : $\boxed{y=3x-2}$.
  \item Racine : $3x-2=0\Leftrightarrow x=\dfrac{2}{3}$. La droite coupe l'axe des $x$ en $\Big(\dfrac23;0\Big)$.
\end{enumerate}

\exercice{5}
$a=\dfrac{5-5}{-1-2}=\dfrac{0}{-3}=0$. La pente est nulle : $\boxed{y=5}$, une droite \textbf{horizontale}. Les deux points ont la même ordonnée $5$, ce qui est cohérent.
\end{document}
