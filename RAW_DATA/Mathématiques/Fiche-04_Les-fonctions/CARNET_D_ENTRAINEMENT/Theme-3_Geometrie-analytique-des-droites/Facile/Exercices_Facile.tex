...
\begin{document}
\titrecarnet{Thème 3 — Géométrie analytique des droites}{Exercices — niveau Facile}{Un outil à la fois : pente, milieu, distance, équation.}

\exercice{1}
Soit $A(1;2)$ et $B(4;8)$.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Calculer la pente $a$ de la droite $(AB)$.
  \item En déduire l'équation $y=ax+b$ de $(AB)$.
\end{enumerate}

\exercice{2}
Calculer les coordonnées du milieu $M$ du segment $[AB]$ avec $A(-2;3)$ et $B(4;-1)$.

\exercice{3}
Calculer la distance $\overline{AB}$ entre $A(0;0)$ et $B(3;4)$.

\exercice{4}
Une droite $d$ a pour pente $3$ et passe par le point $(0;-2)$.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Écrire l'équation de $d$.
  \item Déterminer sa racine (abscisse du point où elle coupe l'axe des $x$).
\end{enumerate}

\exercice{5}
Soit $A(2;5)$ et $B(-1;5)$. Calculer la pente de $(AB)$ et donner l'équation de la droite. Que remarque-t-on ?
\end{document}
