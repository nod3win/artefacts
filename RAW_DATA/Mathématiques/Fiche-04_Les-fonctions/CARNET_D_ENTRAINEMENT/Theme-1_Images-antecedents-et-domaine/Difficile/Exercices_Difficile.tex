...
\begin{document}
\titrecarnet{Thème 1 — Images, antécédents \& domaine}{Exercices — niveau Difficile}{Compositions, pièges et niveau concours.}

\exercice{1}
On considère le trinôme $x^2-x-6$.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Factoriser $x^2-x-6$, puis déterminer le domaine de définition de $f(x)=\sqrt{x^2-x-6}$.
  \item Déterminer le domaine de définition de $g(x)=\ln(x^2-x-6)$. Comparer avec le domaine de $f$.
\end{enumerate}

\exercice{2}
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Déterminer le domaine de définition de $f(x)=\sqrt[3]{2x+1}$ (racine \emph{cubique}).
  \item Déterminer le domaine de définition de $g(x)=\sqrt{2x+1}$ (racine \emph{carrée}).
...
\exercice{4}
Soit $f(x)=\dfrac{x}{x+1}$, définie pour $x\neq -1$.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Montrer que $f\big(f(x)\big)=\dfrac{x}{2x+1}$.
  \item En déduire le domaine de définition de la composée $f\circ f$.
\end{enumerate}

\exercice{5}
\textbf{Mise en situation.} Une grandeur est modélisée par $f(x)=\dfrac{\ln x}{\sqrt{4-x^2}}$. Déterminer l'ensemble des valeurs de $x$ pour lesquelles cette expression a un sens (son domaine de définition).
\end{document}
