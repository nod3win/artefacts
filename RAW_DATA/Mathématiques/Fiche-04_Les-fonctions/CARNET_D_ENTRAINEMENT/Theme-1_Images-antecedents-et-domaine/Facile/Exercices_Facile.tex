...
\begin{document}
\titrecarnet{Thème 1 — Images, antécédents \& domaine}{Exercices — niveau Facile}{Un seul geste à la fois : une image, un antécédent, un interdit.}

\exercice{1}
Soit $f(x)=3x-5$.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Calculer les images $f(2)$, $f(-1)$ et $f(0)$.
  \item Déterminer l'antécédent de $4$ par $f$.
\end{enumerate}

\exercice{2}
Soit $g(x)=x^2-2x$.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Calculer $g(3)$ et $g(-2)$.
  \item Déterminer tous les antécédents de $0$ par $g$.
\end{enumerate}

\exercice{3}
Déterminer le domaine de définition de $f(x)=\dfrac{1}{x-3}$.

\exercice{4}
Déterminer le domaine de définition de $g(x)=\sqrt{x-2}$.

\exercice{5}
Déterminer le domaine de définition de $h(x)=\ln(x+1)$.
\end{document}
