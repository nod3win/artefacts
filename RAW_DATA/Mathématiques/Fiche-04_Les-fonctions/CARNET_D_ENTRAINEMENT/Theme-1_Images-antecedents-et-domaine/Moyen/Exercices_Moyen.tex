...
\begin{document}
\titrecarnet{Thème 1 — Images, antécédents \& domaine}{Exercices — niveau Moyen}{Combiner les interdits ; dresser un tableau de signes.}

\exercice{1}
Déterminer le domaine de définition de $f(x)=\sqrt{2x-6}$.

\exercice{2}
Déterminer le domaine de définition de $f(x)=\dfrac{x+1}{x^2-4}$.

\exercice{3}
Déterminer le domaine de définition de $f(x)=\ln(3-x)$.

\exercice{4}
Déterminer le domaine de définition de $f(x)=\dfrac{1}{\sqrt{x-1}}$.
\emph{Indication : ici deux interdits se cumulent.}

\exercice{5}
Déterminer le domaine de définition de $f(x)=\sqrt{\dfrac{x-1}{x+2}}$.
\emph{Indication : on dresse le tableau de signes du quotient.}
\end{document}
