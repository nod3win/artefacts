...
\begin{document}
\titrecarnet{Thème 2 — Parité, symétries \& périodicité}{Exercices — niveau Moyen}{Axe, centre de symétrie et période.}

\exercice{1}
Soit $f(x)=x^2-6x+5$.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Déterminer l'équation de l'axe de symétrie à l'aide de la formule $x=-\dfrac{b}{2a}$.
  \item Vérifier par le calcul que $f(3-x)=f(3+x)$.
\end{enumerate}

\exercice{2}
Pour les fonctions $\cos$ et $\tan$, préciser la parité \emph{et} la période. Justifier brièvement chaque réponse.

\exercice{3}
Soit $f(x)=x^3+2$. Montrer que le point $(0;2)$ est un centre de symétrie de la courbe de $f$.

\exercice{4}
Montrer que $f(x)=\sin(2x)$ est périodique de période $T=\pi$.

\exercice{5}
Montrer que $f(x)=\dfrac1x$ est impaire, et en déduire un centre de symétrie de son graphe.
\end{document}
