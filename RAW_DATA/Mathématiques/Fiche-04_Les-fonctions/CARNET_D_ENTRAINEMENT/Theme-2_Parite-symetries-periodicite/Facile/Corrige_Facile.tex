...
\begin{document}
\titrecarnet{Thème 2 — Parité, symétries \& périodicité}{Corrigé — niveau Facile}{Détail des calculs.}

\exercice{1}
$f(-x)=(-x)^2-3=x^2-3=f(x)$. Donc $f$ est \textbf{paire} (axe de symétrie : l'axe des $y$).

\exercice{2}
$f(-x)=(-x)^3+(-x)=-x^3-x=-(x^3+x)=-f(x)$. Donc $f$ est \textbf{impaire} (centre de symétrie : l'origine).

\exercice{3}
$f(-x)=2(-x)+1=-2x+1$. Or $f(x)=2x+1$ et $-f(x)=-2x-1$. On a $f(-x)\neq f(x)$ et $f(-x)\neq -f(x)$.
Contre-exemple : $f(1)=3$, $f(-1)=-1$. Donc $f$ n'est \textbf{ni paire ni impaire}.

\exercice{4}
$f(-x)=\lvert -x\rvert=\lvert x\rvert=f(x)$. Donc $f$ est \textbf{paire}.

\exercice{5}
$f(-x)=(-x)^2+(-x)=x^2-x$. Or $f(x)=x^2+x$ et $-f(x)=-x^2-x$. Ni égalité.
Contre-exemple : $f(1)=2$, $f(-1)=0$ : $f(-1)\neq f(1)$ et $f(-1)\neq -f(1)=-2$. Donc $f$ n'est \textbf{ni paire ni impaire}.
\end{document}
