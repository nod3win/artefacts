...
\begin{document}
\titrecarnet{Thème 4 — Fonction du second degré}{Exercices — niveau Difficile}{Racines imposées, intersection, optimisation, étude complète.}

\exercice{1}
On considère le polynôme $P(x)=ax^2+bx+c$. On sait que $-1$ et $2$ sont racines de $P$ et que $P(1)=-4$.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Écrire $P(x)$ sous forme factorisée (en fonction de $a$).
  \item Déterminer $a$.
  \item Calculer $P(0)$.
\end{enumerate}

\exercice{2}
Déterminer les coordonnées des points d'intersection de la parabole $y=x^2-3x+1$ et de la droite $y=x+1$.

\exercice{3}
\textbf{Mise en situation (optimisation).} Un éleveur clôture un enclos rectangulaire contre un long mur : le mur forme un côté, et il dispose de $20$~m de grillage pour les \emph{trois} autres côtés (deux largeurs $x$ et une longueur).
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Exprimer la longueur en fonction de $x$, puis l'aire $A(x)$ de l'enclos.
  \item Pour quelle largeur $x$ l'aire est-elle maximale ?
  \item Quelle est cette aire maximale ?
\end{enumerate}

\exercice{4}
\textbf{Étude complète.} Soit $f(x)=x^2-4x+3$.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Domaine, discriminant et racines.
  \item Sommet, axe de symétrie, intersection avec $Oy$, forme factorisée.
  \item Dresser le tableau de variation et tracer l'allure de la courbe.
\end{enumerate}
\end{document}
