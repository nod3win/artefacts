...
\begin{document}
\titrecarnet{Thème 5 — Trigonométrie, exponentielle \& logarithme}{Exercices — niveau Facile}{Valeurs, propriétés, périodes et parités.}

\exercice{1}
Calculer, sans calculatrice : $\log_2 8$, \ $\log_3\dfrac19$, \ $\ln(e^3)$, \ $\ln 1$.

\exercice{2}
Pour les fonctions $\sin$, $\cos$ et $\tan$, donner la période \emph{et} la parité. Justifier brièvement.

\exercice{3}
Simplifier : \ $e^{2x}\cdot e^{3x}$, \quad $(e^x)^2$, \quad $e^{\ln 5}$.

\exercice{4}
Donner les valeurs de $\cos 0$, \ $\sin\dfrac{\pi}{2}$, \ $\cos\pi$, \ $\sin 0$.

\exercice{5}
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Résoudre $\ln x=0$.
  \item Déterminer l'intersection de la courbe de $x\mapsto e^x$ avec l'axe des ordonnées.
\end{enumerate}
\end{document}
