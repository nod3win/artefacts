...
\begin{document}
\titrecarnet{Thème 5 — Trigonométrie, exponentielle \& logarithme}{Corrigé — niveau Facile}{Détail des calculs.}

\exercice{1}
$\log_2 8=3$ car $2^3=8$. \quad $\log_3\dfrac19=-2$ car $3^{-2}=\dfrac19$. \quad $\ln(e^3)=3$ (car $\ln$ et $e$ sont réciproques). \quad $\ln 1=0$.

\exercice{2}
\begin{itemize}[leftmargin=1.6em,topsep=2pt]
  \item $\sin$ : période $2\pi$ ; \textbf{impaire} ($\sin(-x)=-\sin x$).
  \item $\cos$ : période $2\pi$ ; \textbf{paire} ($\cos(-x)=\cos x$).
  \item $\tan$ : période $\pi$ ; \textbf{impaire} ($\tan(-x)=-\tan x$).
\end{itemize}

\exercice{3}
$e^{2x}\cdot e^{3x}=e^{2x+3x}=\boxed{e^{5x}}$. \quad $(e^x)^2=\boxed{e^{2x}}$. \quad $e^{\ln 5}=\boxed{5}$.

\exercice{4}
$\cos 0=1$ ; \ $\sin\dfrac{\pi}{2}=1$ ; \ $\cos\pi=-1$ ; \ $\sin 0=0$.

\exercice{5}
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item $\ln x=0\Leftrightarrow x=e^0=\boxed{1}$.
  \item Intersection avec $Oy$ : $x=0$, $e^0=1$, donc le point $\boxed{(0;1)}$.
\end{enumerate}
\end{document}
