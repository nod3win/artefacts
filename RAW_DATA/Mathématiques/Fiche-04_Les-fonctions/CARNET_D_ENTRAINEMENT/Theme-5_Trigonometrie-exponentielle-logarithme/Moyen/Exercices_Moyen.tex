...
\begin{document}
\titrecarnet{Thème 5 — Trigonométrie, exponentielle \& logarithme}{Exercices — niveau Moyen}{Résolutions, réduction d'angle, réciproques, changement de base.}

\exercice{1}
Résoudre : \ (a) $2^x=8$ ; \quad (b) $e^x=1$ ; \quad (c) $3^x=\dfrac19$.

\exercice{2}
Résoudre : \ (a) $\ln x=2$ ; \quad (b) $\ln(x-1)=0$.

\exercice{3}
Calculer $\cos\dfrac{13\pi}{3}$ en réduisant l'angle (on retranche un nombre entier de tours).

\exercice{4}
Donner les valeurs de $\arcsin 1$, \ $\arccos 0$, \ $\arctan 1$ (en radians).

\exercice{5}
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Exprimer $\log_2 5$ à l'aide du logarithme népérien.
  \item Résoudre $e^{2x}=e^{x+1}$.
\end{enumerate}
\end{document}
