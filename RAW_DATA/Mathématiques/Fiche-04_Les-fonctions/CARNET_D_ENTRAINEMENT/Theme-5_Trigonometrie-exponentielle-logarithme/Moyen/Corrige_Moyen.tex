...
\begin{document}
\titrecarnet{Thème 5 — Trigonométrie, exponentielle \& logarithme}{Corrigé — niveau Moyen}{Détail des calculs.}

\exercice{1}
(a) $2^x=8=2^3\Rightarrow \boxed{x=3}$. \quad (b) $e^x=1=e^0\Rightarrow \boxed{x=0}$. \quad (c) $3^x=\dfrac19=3^{-2}\Rightarrow \boxed{x=-2}$.

\exercice{2}
(a) $\ln x=2\Rightarrow x=e^2$ (et $e^2>0$, cohérent) : $\boxed{x=e^2}$. \\
(b) $\ln(x-1)=0\Rightarrow x-1=e^0=1\Rightarrow x=2$ (et $x-1=1>0$, cohérent) : $\boxed{x=2}$.

\exercice{3}
$\dfrac{13\pi}{3}=\dfrac{12\pi}{3}+\dfrac{\pi}{3}=4\pi+\dfrac{\pi}{3}$. Comme $\cos$ est $2\pi$-périodique, on retranche $2$ tours ($4\pi$) :
\[ \cos\frac{13\pi}{3}=\cos\Big(\frac{\pi}{3}\Big)=\boxed{\dfrac12}. \]

\exercice{4}
$\arcsin 1=\dfrac{\pi}{2}$ (car $\sin\tfrac{\pi}{2}=1$) ; \ $\arccos 0=\dfrac{\pi}{2}$ (car $\cos\tfrac{\pi}{2}=0$) ; \ $\arctan 1=\dfrac{\pi}{4}$ (car $\tan\tfrac{\pi}{4}=1$).

\exercice{5}
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Changement de base : $\log_2 5=\dfrac{\ln 5}{\ln 2}$.
  \item $e^{2x}=e^{x+1}$. L'exponentielle étant injective (strictement croissante) : $2x=x+1\Rightarrow \boxed{x=1}$.
\end{enumerate}
\end{document}
