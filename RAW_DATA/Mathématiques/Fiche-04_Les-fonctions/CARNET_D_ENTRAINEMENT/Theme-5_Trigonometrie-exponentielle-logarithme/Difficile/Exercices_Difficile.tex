...
\begin{document}
\titrecarnet{Thème 5 — Trigonométrie, exponentielle \& logarithme}{Exercices — niveau Difficile}{Croissance exponentielle, formulaire, équations, mise en situation.}

\exercice{1}
\textbf{Mise en situation.} Une culture de cellules croît de manière exponentielle : le nombre de cellules \emph{double} tous les $20$ jours.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Combien de jours faut-il pour que la population soit multipliée par $8$ ?
  \item Et pour être multipliée par $16$ ?
  \item Donner l'expression $N(t)$ du nombre de cellules, où $N_0$ est le nombre initial.
\end{enumerate}

\exercice{2}
On suppose que $x$ est un angle du premier quadrant (donc $\cos x>0$) tel que $\sin x=\dfrac34$.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Calculer $\cos x$.
  \item En déduire $\sin(2x)$.
\end{enumerate}

\exercice{3}
On donne $\sin\alpha=-0{,}8$ avec $\pi<\alpha<\dfrac{3\pi}{2}$ (troisième quadrant).
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Déterminer $\cos\alpha$.
  \item En déduire $\tan\alpha$.
\end{enumerate}

\exercice{4}
Résoudre l'équation $\log_2 x+\log_2(x-2)=3$.
\emph{Ne pas oublier la condition d'existence.}

\exercice{5}
\textbf{Mise en situation (demi-vie).} Un médicament a une demi-vie de $6$~heures : la quantité restante suit $Q(t)=Q_0\cdot\Big(\dfrac12\Big)^{t/6}$.
\begin{enumerate}[label=\alph*),leftmargin=1.8em,topsep=2pt]
  \item Quelle fraction de la dose reste-t-il après $12$~heures ?
  \item Au bout de combien d'heures reste-t-il $\dfrac18$ de la dose initiale ?
\end{enumerate}
\end{document}
