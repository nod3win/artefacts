...
\begin{center}
{\LARGE\bfseries\color{primarydark} Tangente : horizontale, parallèle, perpendiculaire}\\[2pt]
...
\begin{exo}[1]
Soit $f(x)=x^2-4x+1$. En quel point la tangente à $\mathcal{C}_f$ est-elle \textbf{horizontale} ? Donner l'équation de cette tangente.
\end{exo}

\begin{exo}[2]
Soit $f(x)=x^3-3x$. Déterminer les \textbf{abscisses} pour lesquelles la tangente est horizontale, puis les \textbf{points} correspondants de la courbe.
\end{exo}

\begin{exo}[3]
Soit $f(x)=x^2$. Montrer que la tangente à $\mathcal{C}_f$ au point d'abscisse $x_0=1$ est \textbf{perpendiculaire} à la droite $\mathcal{D}:y=-\dfrac{x}{2}+3$.
\end{exo}

\begin{exo}[4]
Une fonction $f$ est dérivable et l'on sait que $f(2)=1$. La tangente à $\mathcal{C}_f$ au point d'abscisse $x=2$ passe aussi par le point $B(4;5)$. Déterminer $f'(2)$.
\end{exo}

\begin{exo}[5]
Soit $f(x)=x^2$. En quel point la tangente à $\mathcal{C}_f$ est-elle \textbf{parallèle} à la droite $\mathcal{D}:y=6x-1$ ?
\end{exo}
