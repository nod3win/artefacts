...
\begin{center}
{\LARGE\bfseries\color{primarydark} Produit et quotient}\\[2pt]
{\color{primary}\rule{0.5\textwidth}{1.2pt}}\\[4pt]
{\small\itshape Niveau Moyen --- règles $(UV)'$ et $(U/V)'$}
\end{center}
\vspace{2mm}

\begin{methode}[réflexe produit / quotient]
Repère d'abord $U$ et $V$, calcule $U'$ et $V'$ à part, puis applique la formule sans en modifier l'ordre :
\[
(UV)'=U'V+UV',\qquad \left(\frac{U}{V}\right)'=\frac{U'V-UV'}{V^{2}}.
\]
\end{methode}

\begin{exo}[1 --- règle du produit]
Dériver $\;y=(x^{2}+1)(x-3)$ à l'aide de la règle du produit.
\end{exo}

\begin{exo}[2 --- produit avec racine]
Dériver $\;y=x\sqrt{x}$ par la règle du produit, puis vérifier le résultat en réécrivant $y=x^{3/2}$.
\end{exo}

\begin{exo}[3 --- quotient affine]
Dériver $\;y=\dfrac{2x-1}{x+3}$.
\end{exo}

\begin{exo}[4 --- quotient avec carré]
Dériver $\;y=\dfrac{x^{2}}{x^{2}+1}$.
\end{exo}

\begin{exo}[5 --- quotient affine]
Dériver $\;y=\dfrac{x+1}{x-1}$.
\end{exo}
